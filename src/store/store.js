    ...
    actions: {
      ...
      setOverlayStatus({ state, commit, dispatch, getters }, isOverlay) {
        if (typeof isOverlay !== 'boolean') {
          //... do something, value is not boolean
          throw new Error(`Error in vuex action - setOverlayStatus. Boolean is required, but passed ${isOverlay}`);
        }
      ...
      setPreventBackNav({ state, commit, dispatch, getters }, value) {
        if (typeof value !== 'boolean') {
          //... do something, value is not boolean
          throw new Error(`Error in vuex action - setPreventBackNav. Boolean is required, but passed ${value}`);
        }
        commit('setPreventBackNav', value);
      },
      navigateBack({ state, commit, dispatch, getters }, value) {
        //... if preventBackNav === true ->
        //... set isOverlayPresent to false
        //... if the isSideBarOpen to false - CHECK - try to close the sidebar
        //... else ->
        //... navigateBack
        //... check what happens at the navigation guard when naigating back - what will be the value of store._meta.previousPage
      },
      openSideBar({ state, commit, dispatch, getters }, value) {
        if (typeof value !== 'boolean') {
          //... do something, value is not boolean
          throw new Error(`Error in vuex action - openSideBar. Boolean is required, but passed ${value}`);
        }
      ...
      closeSideBar({ state, commit, dispatch, getters }, value) {
        if (typeof value !== 'boolean') {
          //... do something, value is not boolean
          throw new Error(`Error in vuex action - closeSideBar. Boolean is required, but passed ${value}`);
        }
